...
export default function App() {
  ...
  // Генерируем случайный набор надписей ORO AI для фона
  // Мы делаем это один раз при загрузке компонента
  const backgroundTexts = useRef([...Array(30)].map((_, i) => ({
    id: i,
    top: `${Math.random() * 100}%`,
    left: `${Math.random() * 100}%`,
    rotate: `${(Math.random() * 90) - 45}deg`, // Случайный наклон от -45 до +45
  })));
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] flex items-center justify-center p-4 font-sans text-white overflow-hidden relative">
      {/* --- НАЧАЛО БЛОКА: ДЕКОРАТИВНЫЙ ФОН --- */}
      
      {/* 1. Глубокий градиент и оранжевые свечения */}
      ...
      {/* 2. НОВЫЙ СЛОЙ: ПОЛУПРОЗРАЧНЫЕ НАДПИСИ ORO AI В РАЗНОБРОС */}
      <div className="absolute inset-0 pointer-events-none -z-5 overflow-hidden opacity-[0.04]">
        {backgroundTexts.current.map(text => (
          <span 
            key={text.id} 
            className="absolute font-black text-4xl text-slate-700 tracking-tighter"
            style={{ 
              top: text.top, 
              left: text.left, 
      ...
      {/* --- КОНЕЦ БЛОКА: ДЕКОРАТИВНЫЙ ФОН --- */}

      {step === 'welcome' && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full bg-slate-950/60 backdrop-blur-2xl p-10 rounded-[2.5rem] shadow-[0_0_60px_-15px_rgba(251,146,60,0.3)] border border-slate-800 text-center relative z-10">
          ...
          <p className="text-slate-400 mb-8 font-medium italic leading-relaxed">Join the evolution of decentralized data. Claim your unique ID.</p>
      ...
      {step === 'result' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center w-full flex flex-col items-center max-w-2xl px-4 relative z-10">
          ...
          <div className="space-y-3 w-full max-w-sm">
            ...
            />
            <button onClick={downloadCard} className="w-full bg-white text-slate-950 font-black py-4 rounded-2xl shadow-xl hover:bg-black hover:scale-[1.02] transition-all uppercase tracking-widest">
              Download ID
            </button>
