  ...
  const downloadCard = () => {
    ...
    toPng(cardRef.current, { cacheBust: true })
      .then((dataUrl) => {
        ...
        link.download = `oro-identity-${discord || 'user'}.png`;
        link.href = dataUrl;
        link.click();
      });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 flex items-center justify-center p-4 font-sans text-slate-800 overflow-hidden relative">
      ...
      {step === 'welcome' && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full bg-white/80 backdrop-blur-xl p-10 rounded-[2.5rem] shadow-2xl border border-white text-center">
          <div className="bg-gradient-to-r from-orange-400 to-yellow-400 px-6 py-2 rounded-2xl mx-auto mb-6 flex items-center justify-center shadow-lg shadow-orange-100 w-fit">
            <span className="text-white font-black text-2xl tracking-tighter">ORO</span>
          </div>
          <h1 className="text-3xl font-extrabold mb-2 tracking-tight italic">Intelligence Quiz</h1>
          <p className="text-slate-500 mb-8">Test your knowledge and claim your Digital Identity Card.</p>
        ...
        <div className="w-full max-w-2xl">
          <div className="flex justify-between items-center mb-6 px-4">
            <div className="flex flex-col">
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Node Check</span>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={current} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
              className="bg-white/90 backdrop-blur-2xl p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-white">
      ...
      {step === 'result' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center w-full max-w-sm">
          <div ref={cardRef} className="bg-white p-8 rounded-[2.5rem] shadow-2xl border border-white overflow-hidden relative mb-6">
            <div className="absolute top-0 inset-x-0 h-2 bg-gradient-to-r from-blue-400 to-orange-400" />
            <p className="text-[10px] font-black tracking-[0.3em] text-slate-300 mb-6 uppercase">Certificate of Intelligence</p>
            
            <div className="w-20 h-20 bg-slate-50 rounded-full mx-auto mb-4 border-4 border-white shadow-md flex items-center justify-center overflow-hidden">
              {avatar ? <img src={avatar} alt="avatar" className="w-full h-full object-cover" /> : <span className="text-3xl font-bold text-orange-200 italic">ORO</span>}
            </div>

            <h3 className="text-4xl font-black text-slate-800 mb-1">{score} <span className="text-lg text-slate-300">/ 15</span></h3>
            <p className="text-orange-500 font-bold text-[10px] uppercase tracking-[0.2em] mb-6">Master Data Contributor</p>
            
            <div className="bg-slate-50 rounded-xl p-3 mb-4 border border-slate-100 text-left">
              <p className="text-[8px] text-slate-400 uppercase font-black mb-1">Discord Holder</p>
              <p className="text-sm font-mono font-bold text-slate-700 truncate">{discord || 'Anonymous_Explorer'}</p>
            </div>

            <div className="py-2 px-6 bg-slate-900 text-white rounded-lg inline-block text-[10px] font-bold tracking-widest">
              ORO_VALIDATED_2026
            </div>
          </div>
